const handleDragEnd = (e, draggedItem, fromKey, divRefs) => {
  ...
  for (const key of availableKeys) {
    ...
    console.log(key)
    if (!container) { // unmount huda can make available keys null or undefined when unmount 
      continue;
    }
    if (container.contains(elemUnderMouse)) {
      targetKey = key; // recyclebin, data, etc
  ...
  try {
    ...
    const newSourceData = sourceData.filter(item => item.name !== draggedItem.name);
